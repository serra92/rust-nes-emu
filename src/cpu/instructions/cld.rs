...
pub(in crate::cpu) fn build_instruction_set() -> HashMap<Byte, Instruction> {
    return HashMap::from([
        (Opcode::Imp.into(), Instruction::from(Cpu::cld)),
    ]);
}
...
mod tests {
    ...
    use crate::cpu::{Byte, CpuStatusFlags, instructions::cld};
    ...
    fn test_operation() {
        let (cpu, _) =
            setup_test_with_ps(
                ...
                None,
                CpuStatusFlags::D,
                cld::Opcode::Imp as Byte,
                0x00
            );
        cpu.assert_status(CpuStatusFlags::D, false);
    }
}