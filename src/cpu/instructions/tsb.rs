...
pub(in crate::cpu) fn build_instruction_set() -> HashMap<Byte, Instruction> {
    return HashMap::from([
        (Opcode::Abs.into(), Instruction::from(Cpu::tsb)),
        (Opcode::Zp.into(), Instruction::from(Cpu::tsb)),
    ]);
}

impl crate::cpu::Cpu{
    fn tsb(&mut self, memory: &mut Memory) {
        if let Some(addressing_type) = self.instruction_addressing.get(&self.ir) {
            match addressing_type {
                ...
                _ => {
                    panic!("Missing addressing type for instruction TSB!");
                }
            }
        } else {
            panic!("Missing addressing type for instruction TSB!");
